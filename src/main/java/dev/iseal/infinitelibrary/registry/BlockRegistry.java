...
public class BlockRegistry {
    ...
    private static Block register(Block block, RegistryKey<Block> key, boolean registerItem, RegistryKey<ItemGroup> groupKey) {
        if (registerItem) {
            RegistryKey<Item> itemKey = RegistryKey.of(RegistryKeys.ITEM, key.getValue());
            BlockItem blockItem = new BlockItem(block, new Item.Settings().registryKey(itemKey));
            Registry.register(Registries.ITEM, itemKey, blockItem);
            InfiniteLibraryGroups.addItemToGroup(groupKey, blockItem);
        }

        return Registry.register(Registries.BLOCK, key, block);
    }

    private static Block register(Function<AbstractBlock.Settings, Block> block, AbstractBlock.Settings baseSettings, RegistryKey<Block> key, boolean registerItem, RegistryKey<ItemGroup> groupKey) {
        return register(block.apply(baseSettings.registryKey(key)), key, registerItem, groupKey);
    }

    private static Block register(AbstractBlock.Settings settings, RegistryKey<Block> key, boolean registerItem, RegistryKey<ItemGroup> groupKey) {
        Block block = new Block(settings.registryKey(key));
        return register(block, key, registerItem, groupKey);
    }
