package dev.iseal.infinitelibrary.entities.projectiles;
...
public class ExperienceOrbProjectileEntity extends ProjectileEntity {
    ...
    public ExperienceOrbProjectileEntity(World world, LivingEntity owner, Entity target) {
        this(EntityRegistry.EXPERIENCE_ORB_PROJECTILE, world);
        this.setOwner(owner);
        ...
        this.changeTargetDirection(Direction.Axis.Y);
    }
    ...
    public void tick() {
        ...
        if (this.getWorld().isClient) {
            this.getWorld().addParticle(ParticleTypes.ENCHANT, this.getX() - vec3d.x, this.getY() - vec3d.y + 0.15, this.getZ() - vec3d.z, 0.0, 0.0, 0.0);
    ...
    protected void onEntityHit(EntityHitResult entityHitResult) {
        ...
        boolean bl = player.sidedDamage(damageSource, 4.0F);
        if (bl) {
            if (this.getWorld() instanceof ServerWorld serverWorld) {
                EnchantmentHelper.onTargetDamaged(serverWorld, player, damageSource);
            }

            player.addStatusEffect(new StatusEffectInstance(StatusEffectRegistry.HUBRIS, 200), MoreObjects.firstNonNull(entity2, this));
        }
    }
    ...
    protected void onBlockHit(BlockHitResult blockHitResult) {
        ...
        ((ServerWorld)this.getWorld()).spawnParticles(ParticleTypes.CRIT, this.getX(), this.getY(), this.getZ(), 10, 0.2, 0.2, 0.2, 0.0);
